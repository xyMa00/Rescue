...
class BulkSimulator(object):
    ...
    def create_subsample(self, x, y, celltypes, sparse=False, samp = 2000, count=0):
        ...
        for i in range(no_avail_cts):
            ...
            # df_ct_val = cells_sub.sum(axis=0)
            # # 每个基因在当前细胞类型的平均表达值
            # average_celltype = df_ct_val/len(cells_fraction)
    ...
    def merge_datasets(data_dir="./", files=None, out_name="data.h5ad"):
        ...
        # sorted_columns = sorted(adata.obs.columns, key=sort_key)
        # # 按照排序后的列名重新排列obs
        # adata.obs = adata.obs[sorted_columns]
